...
class Map extends CI_Controller {
    ...
    public function __construct()
	{
		...
		$this->load->model('map_model','map_model');
	}
	...
	public function index(){
        ...
        if ($this->userLevel == 99 || $this->userLevel == 3 || $this->userLevel == 999)
            $data['cluster_list'] = $this->map_model->get_cluster();
        elseif ($this->userLevel == 3)
            $data['cluster_list'] = $this->map_model->get_cluster($this->userid);
        //print_r($data['cluster_list']);


        $this->load->model('clusterlead_model');
		//load view
        $this->load->view('clusterleadheader_map_view',$data);
        //$this->load->view('nav_view');
        $this->load->view('clusterlead_map_view');
    ...
    public function get_site(){
        ...
        if (isset($_GET['term'])){
          ...
          $this->map_model->get_list_site($q);
        }
    }
    ...
    public function get_user(){
        ...
        if (isset($_GET['term'])){
          $q = strtolower($_GET['term']);
          $this->map_model->get_list_user($q);
        }
    }  
    ...
    public function get_cluster($id = null){
        ...
        $this->load->model('map_model');
        if ($id)
            $id = $this->map_model->getClusterByUserID($id);

        $data = $this->map_model->get_cluster($id);
    ...
    public function get_clusterbyuser(){
        ...
        if ($userlevel == 99 || $userlevel == 999)
            $data = $this->map_model->get_cluster();
        elseif ($userlevel == 3)
            //$data = $this->map_model->get_cluster($userid);
            $data = $this->map_model->getClusterByUserID($userid);
    ...
    public function ajax_list()
    {
        ...
        $this->load->model('map_model');
        $list = $this->map_model->get_datatables($_GET);
        //print_r($list);
        $data = array();
        $no = isset($_GET['start']);
        ...
        $output = array(
                        ...
                        "recordsTotal" => $this->map_model->count_all(),
                        "recordsFiltered" => $this->map_model->count_filtered(),
                        "data" => $data,
    ...
    public function test_model_function (){
        ...
        $list = $this->map_model->display_result();
    ...
    {   
        ...
        $this->load->model('map_model');
        $list = $this->map_model->get_listattendance_arranged($_GET);
        $data = array();
        foreach ($list as $key) {
            # code...
            $row = array();
